...
class NutricionistaRepository(BaseRepository):
    async def salvar(self, nutricionista: Nutricionista):
        query = """
        INSERT INTO nutricionista (latitude, longitude)
        VALUES ($1, $2)
        """
        await self.db.execute(query, nutricionista.latitude, nutricionista.longitude)
    ...
    async def update(self, id_nutricionista: int, nutricionista: Nutricionista):
        ...
        """
        await self.db.execute(query, id_nutricionista, nutricionista.latitude, nutricionista.longitude)
